...
export function useStrikeCanvas(canvasRef, arenaRef) {
  const fireAnimation = useCallback((name, direction = 'outgoing') => {
    ...
    if (direction === 'outgoing') {
      ...
      else if (name === 'rocket')  animRocket(ctx, canvas, arena, left.x, left.y, right.x, right.y, col);
      ...
      else if (name === 'rocket')  animRocket(ctx, canvas, arena, right.x, right.y, left.x, left.y, col);
