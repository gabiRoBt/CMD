...
export const sounds = {
    ...
    draw: () => {
        if (isSfxMuted()) return;
        const audio = new Audio('/assets/sounds/peace.mp3');
        audio.volume = 0.5;
        audio.play().catch(e => console.warn('draw could not auto-play: ' + e.message));
    },
    war: () => {
        if (isSfxMuted()) return;
        const audio = new Audio('/assets/sounds/war.mp3');
        audio.volume = 0.6;
        audio.play().catch(e => console.warn('war could not auto-play: ' + e.message));
    },
};
